...
#include "wwf.h"  // For Socket and SocketException
/*
...
wwf::wwf(){
    servAddress = "csi-info.baylor.edu";
    servPort    = 80;
  }

string wwf::sendMessage(string command){
      try{

        //Fix problem with spaces in the command
        int i  = 0;
        ...
        message = "GET /wwf/script.php?" + command + "  HTTP/1.0\n\n";
//cerr << message << endl;
        msgLength = message.size();
//cerr << "msgLength = " << msgLength << endl;
        this->socket = new TCPSocket(servAddress, servPort);
        socket->send(message.c_str(), message.size());
        bytesReceived = 0;
        totalBytesReceived = 0;
        while (totalBytesReceived < message.size()) {
          // Receive up to the buffer size bytes from the sender
          if ((bytesReceived = (socket->recv(buffer, BUFSIZE))) <= 0) {
      ...
      delete socket;
//cerr << buffer << endl;
      char* p = strstr(buffer,"Content-Type: text/html");
      p += strlen("Content-Type: text/html\n\r\n\r");
      //outBuff << p;
      return p;
  }

bool wwfstream::operator << (string m){
     outBuff.clear();
     return(outBuff << this->sendMessage(m));
}
...
bool wwfstream::getline(string& m, char c){
    std::getline(outBuff,m,c);
    if(outBuff.fail()){
      outBuff.clear();
      return false;
    }
